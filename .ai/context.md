...
- O pipeline de rascunhos de lição (`scripts/generate-lesson.ts`) foi aposentado em 2026-09-19 junto com o pipeline Piper/whisper.cpp — todos os scripts associados foram deletados. Produção de áudio agora usa o fluxo simples de `docs/09-PADRAO-DE-LICOES.md` (qualquer IA de voz). Revisão humana antes de publicar continua obrigatória.
...
- Roteiros de narração (`narration.script`) escritos para as 48 cenas do currículo (12 lições × 4 cenas) em 2026-09-12. O pipeline de mídia (`scripts/media/`) foi aposentado em 2026-09-19 — os 48 MP3s foram regenerados com edge-tts (voz `pt-BR-AntonioNeural`) e integrados diretamente. Ver `docs/07-CHANGELOG.md` e `docs/18-SUBSTITUICAO-NARRACAO-COMPLETA.md`.
...
- Ainda em 2026-09-13, deploy no ar: os 6 secrets `VITE_FIREBASE_*` foram criados no GitHub e o workflow `firebase-hosting-merge.yml` rodou com sucesso — Estrelar FREE publicado no Firebase Hosting. Na sequência, auditoria de segurança completa do app (código, Firestore rules, dependências, configuração de deploy). Achado real corrigido: `firebase.json` usava o `ignore` padrão da Firebase CLI (`**/.*`), que bloquearia silenciosamente `/.well-known/assetlinks.json` quando a publicação Android (`docs/15-PUBLICACAO-ANDROID.md`) precisar dele — removido. Hardening adicional aplicado: headers de segurança (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`) adicionados ao Hosting; `"firestore": {"rules": "firestore.rules"}` adicionado ao `firebase.json` para `firebase deploy` manter as regras versionadas em sincronia com o que está publicado (antes só existiam publicadas manualmente pelo console); `.firebase/` (cache local da CLI) adicionado ao `.gitignore`; `permissions: contents: read` adicionado ao workflow de merge (privilégio mínimo do `GITHUB_TOKEN`). Um achado inicial foi descartado após checar `legacy/README.md`: os arquivos `legacy/**/*.example` com valor `REDACTED` já haviam sido sanitizados de propósito pela sessão que arquivou `legacy/` em 2026-09-12 — não são credenciais reais. `npm audit` (0 vulnerabilidades) e varredura de padrões de XSS/segredos no código não encontraram mais problemas.
- Ainda em 2026-09-13, a pedido do usuário ("resolver o bd para salvar progresso da conta"), resolvido o mecanismo técnico da `DECISÃO NECESSÁRIA` "persistência de progresso" (`docs/08-ARQUITETURA-PEDAGOGICA.md`): o progresso da jornada (onboarding, lições, missões, XP), que só existia em `localStorage`, agora também é salvo por conta no Firestore (`users/{uid}/progress/journey`, documento separado da identidade em `users/{uid}`). `firestore.rules` ganhou a regra correspondente. `JourneyProvider` foi reescrito para buscar o progresso da conta ao logar (migrando progresso local pré-existente se o Firestore ainda não tiver nada) e gravar local + remotamente a cada ação; o cache local passou a ser por conta (`estrelar-journey-v2:{uid}`, antes era uma chave global única). Só o mecanismo técnico foi decidido — formato definitivo de XP, missão e quiz continuam em aberto, sem mudança de estrutura de dados. `docs/05-BANCO-DE-DADOS.md` e `docs/08-ARQUITETURA-PEDAGOGICA.md` atualizados; detalhes técnicos completos em `docs/07-CHANGELOG.md`. Validado com `npm run lint` e `npm run build`, ambos limpos; teste do fluxo real de sincronização no Firestore fica para o usuário confirmar no próprio deploy.
- Em 2026-09-14, implementado o onboarding real de `docs/02-ROADMAP.md` (P0): `OnboardingPage.tsx` agora coleta nome e segmento do negócio (texto livre) antes de concluir o onboarding, gravados em `users/{uid}` (`saveBusinessProfile`, `docs/05-BANCO-DE-DADOS.md`). Validado com `npm run lint` e `npm run build`, ambos limpos. Detalhes em `docs/07-CHANGELOG.md`.
- Em 2026-09-13/14, implementada a landing page pública (P0): rota `/` para deslogados via `AuthGate` (`src/app/App.tsx`), 10 seções em `src/components/landing/` (`LandingPage.tsx`), SEO básico + Open Graph em `index.html` (`public/og-image.jpg`). Ver `docs/07-CHANGELOG.md`.
- Em 2026-09-19, commit externo `1e2db6d` ("melhorias completas de qualidade", feito fora das sessões anteriores e verificado nesta: `lint` limpo, `build` ok, 43/43 testes, `npm audit` 0 vuln): **narração 48/48 integrada em MP3** (edge-tts, voz única `pt-BR-AntonioNeural` — lote N1–N48 de `docs/18` concluído, `.wav` apagados), code splitting (Firestore via dynamic import, 4 rotas `lazy`, entry ~355 KB), CSS Modules em 4 componentes, 43 testes (Vitest), focus trap no menu mobile, `README.md` reescrito. **Falta a aprovação por escuta dos 48 áudios** (checklist em `docs/18`). Ver `docs/07-CHANGELOG.md`.
...
- **P0 — Produto Free:** ✅ currículo (12 lições, narração em áudio nas 48 cenas em MP3 com voz única, 2026-09-19), ✅ QR Code/link de avaliações, ✅ onboarding (coleta de nome/segmento do negócio, 2026-09-14), ✅ landing page pública (2026-09-13/14) e ✅ deploy real no Firebase Hosting (2026-09-13) concluídos; resta a aprovação por escuta dos áudios (checklist em `docs/18-SUBSTITUICAO-NARRACAO-COMPLETA.md`) e as dívidas não-técnicas da auditoria editorial (`docs/16-AUDITORIA-EDITORIAL-LICOES.md`).
...
3. colocar o app no ar — projeto Firebase real ✅ concluído em 2026-09-12; deploy no Firebase Hosting (decisão de 2026-09-13) ✅ publicado em 2026-09-13.
...
**Fase 3 (Academia/conteúdo educacional) NÃO está mais pausada.** Havia sido pausada em 2026-08-24 a pedido explícito do usuário; a pausa foi encerrada em 2026-09-12, quando o usuário definiu lições como prioridade nº 1 (ver "Foco vigente" acima). A produção de lições pode ser retomada normalmente — o pipeline antigo foi aposentado, agora usa o fluxo simples de `docs/09-PADRAO-DE-LICOES.md`.

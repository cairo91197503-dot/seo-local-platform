...
- Em 2026-09-13, o usuário gerou (via Gemini) e pediu para integrar as 27 imagens restantes (lições 6, 8-12 completas, mais as 3 cenas que faltavam da lição 7). Feito: as 12 lições do currículo agora têm imagem nas 4 cenas. Roteiro de narração já está escrito para as 12 lições; a integração de áudio (Piper/whisper.cpp) está em progresso entre a VM Oracle do usuário e o restante do projeto — só a cena `intro` de `reviews-importance` tem áudio confirmado integrado, o status das demais cenas segue "a confirmar" até sincronizar o trabalho da VM. Ver `docs/13-CURRICULO-MVP.md` e `docs/07-CHANGELOG.md`.
...
1. ✅ currículo/lições (Fase 3) — concluído em 2026-09-12: as 12 lições e 12 missões do currículo do MVP (`docs/13-CURRICULO-MVP.md`) estão implementadas em texto. Produção audiovisual: as 12 lições têm imagem nas 4 cenas desde 2026-09-13; roteiros de narração (`narration.script`) escritos para as 48 cenas e pipeline de mídia habilitado para as 12 lições — próximo passo é confirmar e concluir a integração do áudio de verdade (Piper/whisper.cpp, em progresso na VM Oracle);

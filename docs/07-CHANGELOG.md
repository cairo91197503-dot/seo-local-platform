...
**O que mudou:** novo módulo `src/state/journeyRemote.ts` (`fetchJourneyProgress`/`saveJourneyProgress`) fala com o Firestore. `src/state/journey.ts` passou a versionar o cache local por conta (`estrelar-journey-v2:{uid}`, antes era uma única chave global) e expõe `isJourneyState` para validar o que vem do Firestore. `JourneyProvider` foi reescrito: ao logar, busca o documento da conta no Firestore (fonte de verdade); se não existir ainda, migra o que já houver no cache local desta conta ou na chave global antiga (evita perder progresso de testes já feitos antes desta mudança) e grava esse valor inicial no Firestore; a partir daí, toda ação (concluir lição, iniciar/confirmar missão etc.) grava local e remotamente. Enquanto o Firestore ainda não respondeu para a conta atual, a tela já reflete a ação normalmente, mas a gravação fica pendente até a leitura inicial confirmar (evita sobrescrever, com um estado parcial, um progresso remoto que ainda está sendo buscado). `firestore.rules` ganhou a regra correspondente (`users/{userId}/progress/{progressId}`, mesma restrição de dono da conta do documento de identidade). `docs/05-BANCO-DE-DADOS.md` e `docs/08-ARQUITETURA-PEDAGOGICA.md` atualizados.

Validado com `npm run lint` e `npm run build`, ambos limpos. Teste manual do fluxo de login/sincronização real (conferir no console do Firebase que `users/{uid}/progress/journey` é criado e atualizado) ainda depende do usuário, no próprio deploy.
...
**PWA implementada** (pré-requisito técnico do TWA, e uma melhoria standalone do app web): `vite-plugin-pwa` instalado e configurado em `vite.config.ts`, gerando `manifest.webmanifest` e service worker (Workbox) a cada build. Ícones gerados a partir do `favicon.svg` existente (script novo: `scripts/gen-icons.mjs`, usa `sharp`) em `public/icons/` — versões `any` (192/512) e `maskable` (192/512, com padding pra caber na safe zone de 80% que o Android usa ao recortar ícones), mais `apple-touch-icon.png`. `index.html` ganhou `theme-color`, `description` e o link do apple-touch-icon. Cores do manifest usam `#faf6ef`, o mesmo `--color-bg` já usado no app inteiro. Validado com `npm run build` (manifest/service worker/ícones gerados corretamente), `npm run preview` (content-types corretos) e `npm run lint` (limpo).

**Guia completo criado:** `docs/15-PUBLICACAO-ANDROID.md` documenta o passo a passo restante — deploy no Render, escrever e publicar política de privacidade (exigida pela Play Store por causa do login), rodar `bubblewrap init`/`bubblewrap build` pra gerar o pacote assinado, publicar `assetlinks.json` para verificação de domínio, e submeter na Play Console (conta de desenvolvedor, ficha da loja, classificação de conteúdo, formulário de segurança de dados).

## 2026-09-13 — Correção: narração em áudio restaurada em 6 lições após conflito com a integração de imagens

A pedido do usuário ("da uma conferida nas liçoes e narraçao"), foi feita uma auditoria comparando o repositório local (usado para integrar as imagens da entrada anterior) com o `origin/main` real do GitHub. Encontrado um problema: entre a integração de imagens desta sessão e o momento em que o usuário deu `git push`, a VM Oracle já tinha gerado e integrado a narração em áudio de 44 das 48 cenas (commit `feat: integra narração de áudio (Piper TTS) nas lições e manifests de mídia`, enviado ao GitHub pela sessão de Claude Code rodando na VM). Como a sessão que integrou as imagens partiu de uma cópia local desatualizada (sem esse commit de áudio), os campos `audioSrc`/`segments`/`estimatedDurationSeconds` de 6 lições foram sobrescritos com versões antigas sem áudio ao aplicar só o campo `illustration` — apagando, no `.ts` de cada lição (não nos arquivos de áudio em si, que continuaram intactos em `public/audio/`), a referência ao áudio já gerado.

**Lições afetadas e corrigidas:** `reviews-importance` (cenas trust/timing/action), `first-profile-checkup`, `how-to-respond-to-reviews`, `keep-your-profile-updated`, `photos-help-customers-decide` e `review-request-message` (4 cenas cada). Corrigido com um merge de três vias (`git merge-file`, usando como base o commit anterior às duas mudanças) entre a versão com `illustration` e a versão com `audioSrc`/`segments`/`estimatedDurationSeconds`, sem conflitos — os dois conjuntos de campos ocupam blocos diferentes de cada cena. Validado com `npm run lint` e `npm run build`, ambos limpos, e conferido campo a campo contra o commit de origem do áudio antes de aplicar.

**Status real do áudio depois da correção:** 43 das 48 cenas do currículo têm narração em áudio integrada. Faltam só a cena "Agora é sua vez" da lição 4 (`business-hours-matter`) e as 4 cenas da lição 12 (`choose-your-next-action`) — nenhuma delas foi afetada por este problema; simplesmente a VM ainda não tinha chegado a gerar o áudio dessas quando a auditoria foi feita. `docs/13-CURRICULO-MVP.md` atualizado com o status real por lição.

...
**As 12 lições e 12 missões do currículo estão implementadas**, e as lições 1 a 5 (mais a cena 1 da lição 7) já têm imagem — nenhuma lição usa áudio no momento (narração removida da lição 7 em 2026-09-12, por decisão do usuário). Produção de imagem para as lições 6 e 8 a 12 segue pendente, lição por lição, via os prompts em `docs/14-PROMPTS-DE-IMAGEM-LICOES.md`. Próximo passo pedagógico: revisão humana do texto de cada lição nova, e as prioridades nº 2 (QR Code) e nº 3 (app no ar) do roadmap.

...
- Ilustração: em 2026-09-12, as lições 1 a 5 (`why-appear-in-local-search`, `profile-represents-your-business`, `accurate-business-info`, `business-hours-matter`, `explain-what-you-offer`) ganharam imagem em todas as 4 cenas, geradas manualmente via Gemini a partir de prompts preparados em `docs/14-PROMPTS-DE-IMAGEM-LICOES.md` e revisadas pelo usuário. A lição 7 (`reviews-importance`) mantém a imagem já aprovada da cena 1, mas a narração/áudio dessa cena foi removida por decisão do usuário ("vamos tirar a narração por enquanto") — nenhuma lição usa áudio no momento. As lições 6 e 8 a 12 seguem sem imagem — e, como nenhuma lição tem legenda visível (ver abaixo), essas ficam sem nenhum conteúdo perceptível na tela além do fundo. Produção de imagem para as lições restantes é trabalho separado, via os prompts já preparados em `docs/14` ou `scripts/generate-lesson.ts` (`docs/09`), lição por lição.
- Legenda visível: **removida de todas as 12 lições em 2026-09-12**, a pedido do usuário ("retirar as legendas e deixar apenas narração") — `title` e `text` de cada cena deixaram de aparecer na tela; a interface (`SceneView`) agora mostra só a ilustração (quando existir) e o controle de áudio (quando existir), com o texto preservado apenas como transcript para leitores de tela. Ver `docs/09-PADRAO-DE-LICOES.md` e `docs/07-CHANGELOG.md`.
...
| 12 | 5. Descubra o que melhorar | Escolha sua próxima ação (`choose-your-next-action`) | Fechar o currículo mínimo transformando reflexão em rotina: escolher e executar uma melhoria concreta. | Lição 11 | Execute uma melhoria escolhida (`execute-one-improvement`) | O usuário executa uma das 3 melhorias identificadas na missão anterior. | **Implementada** (imagem nas 4 cenas; roteiro de narração escrito; sem legenda visível — áudio sendo produzido separadamente, status a confirmar) |

**As 12 lições e 12 missões do currículo estão implementadas**, e agora todas as 12 lições têm imagem nas 4 cenas. O roteiro de narração também já está escrito para as 12 lições. A integração de áudio (narração gravada + legenda sincronizada) está em progresso via o pipeline de mídia (`scripts/media/`, rodando tanto localmente quanto numa VM Oracle com Piper/whisper.cpp) — a lição 7 (`reviews-importance`) já tem a cena 1 com áudio integrado; as demais cenas e lições ainda dependem de sincronizar o trabalho feito na VM com o restante do repositório antes de confirmar status final por lição. Nenhuma lição tem legenda visível no momento — a legenda foi removida da interface em 2026-09-12 a pedido do usuário, antes de haver narração pronta para substituí-la. Finalizar a integração de áudio em todas as 12 lições é o próximo passo pendente para essa experiência ficar completa, junto com as prioridades nº 2 (QR Code) e nº 3 (app no ar) do roadmap.

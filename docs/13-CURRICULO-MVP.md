...
| 12 | 5. Descubra o que melhorar | Escolha sua próxima ação (`choose-your-next-action`) | Fechar o currículo mínimo transformando reflexão em rotina: escolher e executar uma melhoria concreta. | Lição 11 | Execute uma melhoria escolhida (`execute-one-improvement`) | O usuário executa uma das 3 melhorias identificadas na missão anterior. | **Implementada** (sem imagem, sem legenda visível, sem áudio) |

**As 12 lições e 12 missões do currículo estão implementadas**, e as lições 1 a 5 (mais a cena 1 da lição 7) já têm imagem. Nenhuma lição tem legenda visível ou áudio no momento — a legenda foi removida da interface em 2026-09-12 a pedido do usuário, antes de haver narração pronta para substituí-la, então hoje a maioria das cenas está sem nenhum conteúdo perceptível além da imagem (quando existir). Produção de imagem (lições 6 e 8 a 12, via `docs/14-PROMPTS-DE-IMAGEM-LICOES.md`) e de narração em áudio (todas as 12, via `scripts/generate-lesson.ts`/Piper na VM Oracle) são os próximos passos pendentes para essa experiência ficar completa, junto com as prioridades nº 2 (QR Code) e nº 3 (app no ar) do roadmap.
